...
from utils.paddle_utils import init_seeds
...
# Prevent OpenCV from multithreading (to use Paddle DataLoader)
cv2.setNumThreads(0)
...
def check_anchors(dataset, model, thr=4.0, imgsz=640):
    ...
    def metric(k):  # compute metric
        ...
        aat = (x > 1. / thr).astype('float32').sum(1).mean()  # anchors above threshold
        bpr = (best > 1. / thr).astype('float32').mean()  # best possible recall
        return bpr, aat
    ...
    if bpr < 0.98:  # threshold to recompute
        ...
        if new_bpr > bpr:  # replace anchors
            new_anchors = paddle.to_tensor(new_anchors).type_as(m.anchors)
            m.anchor_grid[:] = new_anchors.clone().view_as(m.anchor_grid)  # for inference
            m.anchors[:] = new_anchors.clone().view_as(m.anchors) / m.stride.reshape([-1, 1, 1])  # loss
...
def check_anchor_order(m):
    ...
    a = m.anchor_grid.prod(-1).reshape([-1])  # anchor area
    da = a[-1] - a[0]  # delta a
...
def xyxy2xywh(x):
    ...
    y = paddle.zeros_like(x) if isinstance(x, paddle.Tensor) else np.zeros_like(x)
...
def scale_coords(img1_shape, coords, img0_shape, ratio_pad=None):
    ...
    if ratio_pad is None:  # calculate from img0_shape
        ...
        gain = ratio_pad[0][0]
        pad = ratio_pad[1]
    coords_numpy = coords.numpy()
    coords_numpy[:, [0, 2]] -= pad[0]  # x padding
    coords_numpy[:, [1, 3]] -= pad[1]  # y padding
    coords_numpy[:, :4] /= gain
    coords = paddle.to_tensor(coords_numpy)
...
def box_iou(box1, box2):
    ...
    inter = (paddle.minimum(paddle.to_tensor(box1.numpy()[:, None, 2:]), box2[:, 2:]) - paddle.maximum(paddle.to_tensor(box1.numpy()[:, None, :2]), box2[:, :2])).clip(0).prod(2)
    return inter / (paddle.to_tensor(area1.numpy()[:, None]) + area2 - inter)  # iou = inter / (area1 + area2 - inter)
...
class FocalLoss(nn.Layer):
    ...
    def forward(self, pred, true):
        ...
        pred_prob = paddle.nn.functional.sigmoid(pred)  # prob from logits
...
class BCEBlurWithLogitsLoss(nn.Layer):
    ...
    def forward(self, pred, true):
        loss = self.loss_fcn(pred, true)
        pred = paddle.nn.functional.sigmoid(pred)  # prob from logits
...
def compute_loss(p, targets, model):  # predictions, targets, model
    ...
    for i, pi in enumerate(p):  # layer index, layer predictions
        ...
        if n:
            ...
            # Regression
            pxy = paddle.nn.functional.sigmoid(ps[:, :2]) * 2. - 0.5
            pwh = (paddle.nn.functional.sigmoid(ps[:, 2:4]) * 2) ** 2 * anchors[i]
            pbox = paddle.concat((pxy, pwh), 1)  # predicted box
            giou = bbox_iou(pbox.t(), tbox[i], x1y1x2y2=False, CIoU=True)  # giou(prediction, target)
            lbox += (1.0 - giou).mean()  # giou loss
            # Objectness
            # tobj[b, a, gj, gi] = (1.0 - model.gr) + model.gr * giou.detach().clip(0).astype(tobj.dtype)  # giou ratio
            tobj_numpy = tobj.numpy()
            tobj_numpy[b.numpy(), a.numpy(), gj.numpy(), gi.numpy()] = ((1.0 - model.gr) + model.gr * giou.detach().clip(0).astype(tobj.dtype)).numpy()  # giou ratio
            tobj.set_value(tobj_numpy)

            # Classification
            if model.nc > 1:  # cls loss (only if multiple classes)
                t = paddle.full_like(ps[:, 5:], cn)  # targets
                t[range(n), tcls[i]] = cp
    ...
    loss = lbox + lobj + lcls
    return loss * bs, paddle.concat((lbox, lobj, lcls, loss)).detach()
...
def build_targets(p, targets, model):
    ...
    gain = np.ones(6)  # normalized to gridspace gain
    off = np.array([[1, 0], [0, 1], [-1, 0], [0, -1]]).astype('float32')  # overlap offsets

    g = 0.5  # offset
    multi_gpu = False
    for i, jj in enumerate(model.module.yolo_layers if multi_gpu else model.yolo_layers):
        # get number of grid points and anchor vec for this yolo layer
        anchors = model.module.module_list[jj].anchor_vec.numpy() if multi_gpu else model.module_list[jj].anchor_vec.numpy()
        gain[2:] = np.array(p[i].shape)[[3, 2, 3, 2]]  # xyxy gain

        # Match targets to anchors
        a, t, offsets = [], targets * gain, 0
        if nt:
            na = anchors.shape[0]  # number of anchors
            at = np.tile(np.arange(na).astype('float32').reshape(na, 1), [1, nt])
            r = t[None, :, 4:6] / anchors[:, None]  # wh ratio
            j = np.maximum(r, 1. / r).max(2) < model.hyp['anchor_t']  # compare
            # j = wh_iou(anchors, t[:, 4:6]) > model.hyp['iou_t']  # iou(3,n) = wh_iou(anchors(3,2), gwh(n,2))
            a, t = at[j], np.tile(t, [na, 1, 1])[j]  # filter
            ...
            a, t = np.concatenate((a, a[j], a[k], a[l], a[m]), 0), np.concatenate((t, t[j], t[k], t[l], t[m]), 0)
            offsets = np.concatenate((z, z[j] + off[0], z[k] + off[1], z[l] + off[2], z[m] + off[3]), 0) * g
        ...
        # Append
        indices.append((paddle.to_tensor(b), paddle.to_tensor(a, dtype='int64'), paddle.to_tensor(gj.clip(0, gain[3]-1), dtype='int64'), paddle.to_tensor(gi.clip(0, gain[2]-1), dtype='int64')))
        tbox.append(paddle.to_tensor(np.concatenate((gxy - gij, gwh), 1), dtype='float32'))  # box
        anch.append(paddle.to_tensor(anchors[a.astype('int')]))  # anchors
...
def non_max_suppression(prediction, conf_thres=0.1, iou_thres=0.6, merge=False, classes=None, agnostic=False):
    ...
    """
    if prediction.dtype is paddle.float16:
        prediction = prediction.astype('float32')  # to FP32
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        x = paddle.to_tensor(x.numpy()[xc.numpy()[xi]])  # confidence
        ...
        if multi_label:
            if (x[:, 5:] > conf_thres).numpy().sum() == 0:
                x = np.array([])
            else:
                i, j = (x[:, 5:] > conf_thres).nonzero(as_tuple=False).t()
                if box[i].shape == [4]:
                    x = paddle.concat((box[i].unsqueeze(0), paddle.to_tensor(x.numpy()[i.numpy(), j.numpy() + 5, None]), j.unsqueeze(1).astype('float32')), 1)
                else:
                    x = paddle.concat((box[i], paddle.to_tensor(x.numpy()[i.numpy(), j.numpy() + 5, None]), j.unsqueeze(1).astype('float32')), 1)
        else:  # best class only
            conf, j = x[:, 5:].max(1, keepdim=True)
            x = paddle.concat((box, conf, j.astype('float32')), 1)[conf.reshape([-1]) > conf_thres]
        # Filter by class
        if classes:
        ...
        i = paddle_nms(boxes, scores, iou_thres) # 运行太久
        ...
        if merge and (1 < n < 3E3):  # Merge NMS (boxes merged using weighted mean)
            try:  # update boxes as boxes(i,4) = weights(i,n) * boxes(n,4)
                ...
                x[i, :4] = paddle.matmul(weights, x[:, :4]).astype('float32') / weights.sum(1, keepdim=True)  # merged boxes
                if redundant:
...
def box_diou(boxes1, boxes2):
    ...
    inter = (rb - lt).clip(min=0).prod(2)  # [N,M]
...
def non_max_suppression2(prediction, conf_thres=0.1, iou_thres=0.6, max_box=1500, merge=False, classes=None, agnostic=False):
    ...
    t = time.time()
    output = [None] * prediction.shape[0]   
    pred1 = (prediction < -1).astype('float32')[:,:max_box,:6]    # pred1.size()=[batch, max_box, max_box] denotes boxes without offset by class
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if multi_label:
            i, j = (x[:, 5:] > conf_thres).nonzero(as_tuple=False).t()
            x = paddle.concat((box[i], x[i, j + 5, None], j[:, None].astype('float32')), 1)
        else:  # best class only
            conf, j = x[:, 5:].max(1, keepdim=True)
            x = paddle.concat((box, conf, j.astype('float32')), 1)[conf.reshape([-1]) > conf_thres]
        ...
        boxes = (x[:, :4].clone() + c.reshape([-1, 1]) * max_wh)[:max_box]  # boxes (offset by class), scores
        pred2[xi,:] = paddle.concat((boxes, pred2[xi,:]), 0)[:max_box]        # If less than max_box, padding 0.
        pred1[xi,:] = paddle.concat((x[:max_box], pred1[xi,:]), 0)[:max_box]
    ...
    for i in range(200):
        A=B
        maxA=A.max(axis=1)[0]
        E = (maxA<iou_thres).astype('float32').unsqueeze(2).expand_as(A)
        B=iou.mul(E)
        if A.equal(B)==True:
            break
    keep = (maxA <= iou_thres) 
    weights = (B*(B>0.8) + paddle.eye(max_box).cuda().expand([batch_size,max_box,max_box])) * (pred1[:,:,4].reshape((batch_size,1,max_box)))
    pred1[:,:, :4]=paddle.matmul(weights,pred1[:,:,:4]) / weights.sum(2, keepdim=True)   # weighted coordinates
...
def kmean_anchors(path='./data/coco128.yaml', n=9, img_size=640, thr=4.0, gen=1000, verbose=True):
    ...
    def print_results(k):
        ...
        x, best = metric(k, wh0)
        bpr, aat = (best > thr).astype('float32').mean(), (x > thr).astype('float32').mean() * n  # best possible recall, anch > thr
    ...
    wh0 = paddle.to_tensor(wh0, dtype=paddle.float32)  # unflitered
    k = print_results(k)
...
def apply_classifier(x, model, img, im0):
    ...
    for i, d in enumerate(x):  # per image
        if d is not None and len(d):
            ...
            b[:, 2:] = b[:, 2:] * 1.3 + 30  # pad
            d[:, :4] = xywh2xyxy(b).astype('int64')

            # Rescale boxes from img_size to im0 size
            scale_coords(img.shape[2:], d[:, :4], im0[i].shape)

            # Classes
            pred_cls1 = d[:, 5].astype('int64')
            ims = []
...
def plot_wh_methods():  # from utils.utils import *; plot_wh_methods()
    ...
    ya = np.exp(x)
    yb = paddle.nn.functional.sigmoid(paddle.to_tensor(x)).numpy() * 2
