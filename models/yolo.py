...
from utils.paddle_utils import (
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            if not self.training:  # inference
                ...
                y = paddle.nn.functional.sigmoid(x[i])
...
class Model(nn.Layer):
    def __init__(self, cfg='yolov4-p5.yaml', ch=3, nc=None):  # model, input channels, number of classes
        ...
        # print([x.shape for x in self.forward(paddle.zeros([1, ch, 64, 64]))])
    ...
    def _initialize_biases(self, cf=None):  # initialize biases into Detect(), cf is class frequency
        # cf = paddle.bincount(paddle.to_tensor(np.concatenate(dataset.labels, 0)[:, 0]).long(), minlength=nc) + 1.
        ...
        for mi, s in zip(m.m, m.stride):  # from
            ...
            mi.bias = paddle.create_parameter(shape=b.reshape([-1]).shape, dtype='float32',
                           default_initializer=paddle.nn.initializer.Assign(b.reshape([-1])))
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        m_.i, m_.f, m_.type, m_.np = i, f, t, int(np)  # attach index, 'from' index, type, number params
